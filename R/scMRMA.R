...
scMRMA <- function(input,species,db="panglaodb",p=0.05,
                           normalizedData=F,selfDB=NULL,selfClusters=NULL){
  if(!is.null(selfDB)){
  ...
  for (i in 2:ncol(CellType)){
    ...
    if(i==2){
      ...
      if(!is.null(selfClusters)){
        ...
        clusters <- rough_cluster(scdata = scdata,logcounts = logcounts)
      }
          ...
          if(length(unique(marker[,i])) > 1 ){##stop if there is no subtype
            ...
            if(ncol(sub)>20){
              ...
              if(! is.null(selfClusters)){
                ...
                clusters <- precise_cluster(rawdata = sub,logdata = subLog,npcs)
              }
              #print(table(clusters))
              #clusters <- precise_cluster(rawdata = sub,logdata = subLog,npcs)
  ...
  for (i in 1:nrow(anno)) {
    if(anno[i,ncol(anno)] == "Unassigned"){
      if(which(anno[i,]=="Unassigned")[1]>1){anno[i,ncol(anno)]=anno[i,which(anno[i,]=="Unassigned")[1]-1]}
    }
  }
  ...
  uniformR <- uniformR_anno(CellType,scdata,logcounts,p,selfClusters)
...
#'
#' @export
uniformR_anno <- function(CellType,scdata,logcounts,p,selfClusters){
  marker <- CellType
  ...
  if(! is.null(selfClusters)){
    ...
    hvg.gene <- rownames(hvg)[order(hvg$variance.standardized,decreasing = T)][1:2000]
    ...
    #snnMatrix <- getSNN2(knnMatrix,1/15)
...
get_celltype <- function(species,db){
  Species = "Hs"
  if(db == "panglaodb"){
    #load("/Users/lavender/vandy/sample/annotation/database/Human_PanglaoDB.Rdata")
    load(system.file("data", "Human_PanglaoDB.Rdata", package = "scMRMA"))
    if(toupper(species) == "MM"){
      #load("/Users/lavender/vandy/sample/annotation/database/Mouse_PanglaoDB.Rdata")
      load(system.file("data", "Mouse_PanglaoDB.Rdata", package = "scMRMA"))
    }
    return(CellType)
  }
  if(db == "TcellAI"){
    #load(file = "/Users/lavender/vandy/sample/annotation/database/Human_TcellAI.Rdata")
...
#'
#' @export
rough_cluster <- function(scdata,logcounts){
  hvg <- get_variableGene(scdata)
  hvg.gene <- rownames(hvg)[order(hvg$variance.standardized,decreasing = T)][1:2000]
  data.hvg <- logcounts[hvg.gene,]
...
#'
#' @export
precise_cluster <- function(rawdata,logdata,npcs){
  hvg <- get_variableGene(rawdata)
  hvg.top <- hvg[order(hvg$variance.standardized,decreasing = T)[1:2000],]
...
#'
#' @export
selfDefinedDatabase <- function(file){
...
addGene <- function(geneCellTypeList,celltype){
  if(ncol(geneCellTypeList) !=2 ){
    ...
    ct_list <- t(t(t(geneCellTypeList[geneCellTypeList[,2] %in% ct_db,])))
